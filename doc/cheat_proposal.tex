...
\begin{figure}
    \centering
    \subfloat[Person similarity graph. Nodes represent persons, and edges represent persons who are similar. The node set $A$ is a clique, but $B$ is not, since $x$ and $y$ are not connected.]{{\includegraphics[height=5cm]{graph_with_cliques.pdf} }}
    \qquad
    \subfloat[A bi-partite graph. Red nodes represent persons; yellow nodes represent items. An edge is placed only between a person $x$ and an $y$ if $x$'s response to $y$ was aberrant.]{{\includegraphics[height=5cm]{bipartite_graph.pdf} }}
